...
billsRouter.patch('/bills/:id', (req, res) => {
  ...
  for (const k of ['supplier', 'invoiceNumber', 'documentType', 'currency', 'date', 'category', 'categoryReason', 'taxRate', 'description', 'status', 'createdBy', 'paymentMethod', 'customer', 'project']) {
...
billsRouter.post('/bills/:id/finalize', (req, res) => {
  ...
  const b = req.body ?? {};
  const patch: Record<string, unknown> = {};
  for (const k of ['supplier', 'invoiceNumber', 'documentType', 'currency', 'date', 'category', 'categoryReason', 'description']) {
    if (typeof b[k] === 'string') patch[k] = b[k];
  }
  if (b.total != null) patch.total = parseAmount(b.total);
  if (b.tax != null) patch.tax = parseAmount(b.tax);
