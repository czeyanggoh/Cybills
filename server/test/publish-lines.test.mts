...
check('no rows -> null', await perLineItems(bill({ total: '100' }), OPTS), null);

// 2) Rows that don't add up to the bill are never posted as lines.
check(
  'rows short of the total -> null',
  await perLineItems(bill({ total: '1139.05' }), { ...OPTS }) ??
    (await perLineItems(bill({ total: '1139.05', lineItems: [row({ total: '315' }), row({ total: '3045' })] }), OPTS)),
  null
);

// 3) The document states one GST figure: spread it across the rows, exactly.
let lines = (await perLineItems(
  bill({ total: '109', tax: '9', project: 'ASTP 01', lineItems: [row({ total: '60' }), row({ total: '49' })] }),
  OPTS
))!;
...
lines = (await perLineItems(
  bill({ total: '100.10', tax: '0.10', lineItems: [row({ total: '33.37' }), row({ total: '33.37' }), row({ total: '33.36' })] }),
  OPTS
))!;
...
lines = (await perLineItems(
  bill({ total: '218', tax: '18', lineItems: [row({ total: '109', net: '100', tax: '9' }), row({ total: '109', net: '100', tax: '9' })] }),
  OPTS
))!;
...
check(
  'per-row tax that disagrees -> null',
  await perLineItems(bill({ total: '218', tax: '18', lineItems: [row({ total: '109', tax: '9' }), row({ total: '109', tax: '5' })] }), OPTS),
  null
);
...
lines = (await perLineItems(
  bill({ total: '200', lineItems: [row({ total: '100', category: '313A - Outlet Rental' }), row({ total: '100', category: '999 - Gone' })] }),
  OPTS
))!;
...
lines = (await perLineItems(bill({ total: '100', lineItems: [row({ total: '100', description: '' })] }), OPTS))!;
