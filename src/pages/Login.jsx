        ...
        {codes.length ? (
          ...
          <form onSubmit={submitCode} className="space-y-2.5">
            ...
            {/* On by default: asked once, then not again on this machine, for
                good — the trust ends when the person's 2FA is reset or
                re-enrolled, not on a date. Worth unticking on a shared
                computer, which is why it is visible rather than assumed. */}
            <label className="flex items-center justify-center gap-2 text-xs text-muted-foreground">
              <input type="checkbox" checked={trust} onChange={(e) => setTrust(e.target.checked)} className="accent-black" />
              Trust this browser
            </label>
