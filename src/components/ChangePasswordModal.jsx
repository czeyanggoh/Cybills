...
// "Change password" — validates the new password locally and confirms. The
// password itself is never stored client-side; real rotation happens on the
// auth backend later.
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-lg bg-background shadow-xl">
        ...
        <div className="flex items-center justify-end gap-2 border-t px-6 py-4">
          ...
          <button type="button" onClick={save} className="inline-flex h-9 items-center rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground transition-opacity hover:opacity-90">Save</button>
        </div>
      </div>
    </div>
  );
