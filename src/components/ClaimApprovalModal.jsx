...
// automatically to the claimant's direct manager (set per user in Users). The
...
export default function ClaimApprovalModal({ open, onClose, onSubmit, claims = [] }) {
  ...
  const rows = claims.map((c) => ({ claim: c, manager: directManagerFor(users, c.claimFor) }));
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-lg bg-background shadow-xl">
        ...
        <div className="space-y-3 p-6">
          <p className="text-sm text-muted-foreground">
            Each claim is sent to the claimant&rsquo;s <span className="font-medium text-foreground">direct manager</span> for approval, and is visible only to them. Managers are set per user in <span className="font-medium text-foreground">Users</span>.
          </p>
          {routable.length > 0 && (
